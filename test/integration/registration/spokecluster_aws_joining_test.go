...
var _ = ginkgo.Describe("Joining Process for aws flow", ginkgo.Ordered, func() {
	...
	assertJoiningSucceed := func() {
		...
		ginkgo.It("managedcluster should not join with auto approval rejected of managed cluster not in patterns for aws flow", func() {
			...
			// Wait a bit to ensure the hub has had time to process the cluster
			// and make the auto-approval decision
			time.Sleep(5 * time.Second)

			// The ManagedCluster CR should never be accepted since the ARN doesn't match the pattern
