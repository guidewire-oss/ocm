	...
	ginkgo.Context("Deploy hub with aws auth", func() {
					...
					clusterManager.Spec.RegistrationConfiguration.RegistrationDrivers = []operatorapiv1.RegistrationDriverHub{
						{
							...
							AutoApprovedIdentities: []string{"arn:aws:eks:us-west-2:123456789012:cluster/.*, arn:aws:eks:us-west-2:123456789013:cluster/.*"},
						},
					}
				}
		...
		ginkgo.It("should have auto approved arn patterns separated by comma with awsirsa", func() {
			gomega.Eventually(func() bool {
				...
				fmt.Println("Autoapproved patterns %v", autoApprovedArnPatterns)
				return present && autoApprovedArnPatterns == "arn:aws:eks:us-west-2:123456789012:cluster/.*, arn:aws:eks:us-west-2:123456789013:cluster/.*"
			}, eventuallyTimeout, eventuallyInterval).Should(gomega.BeTrue())
		})
	})
})
